...
impl<V, W: Copy> Graph<V, W> {
    ...
    fn _dfs_rec<T, F>(
        ...
        func: &mut F) -> T
    where F: FnMut(NodeSt, &mut V, W, T) -> T, T: Copy + Add<Output=T> {
        ...
        if unvis.contains(&u) {
            ...
            for &e in node_edges[u].iter() {
                ...
                let nnt = Self::_dfs_rec(node_values, node_edges, edges, nu, nw, nt, unvis, func);
                nnnt = nnnt + nnt;
            }

            func(NodeSt::Returned, &mut node_values[u], w, nnnt);

            return nnnt;
        }
        else {
            return func(NodeSt::Visited, &mut node_values[u], w, t);
        }
    }
...
fn main() {
    ...
    g.dfs_rec(Some(*v.first().unwrap()), (), 0, (0..n).collect(),
        |st, vu, _w, t| {
            match st {
                NodeSt::Unvisited => {
                    if v.contains(&vu) {
