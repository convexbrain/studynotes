...
fn main() {
    ...
    for _ in 0..m {
        ...
        let t: u64 = tokens.next();
        g.add_edge(u - 1, v - 1, t);
    }

    let mut d: Vec<Vec<u64>> = Vec::new();
    for i in 0..n {
        g.node_values = vec![u64::MAX; n];
        g.dijkstra(Some(i), (0..n).collect(),
        |v, ws, _p| {
            if *v > ws {
                *v = ws;
                true
    ...
    for _ in 0..q {
        ...
        let b: Vec<usize> = tokens.collect(k);

        let mut tt_min = u64::MAX;

        for p in PermIter::new(k, k).iter() {
            for dir in 0..(1 << k) {
                debug!(p, dir);
                let mut tt = 0;
                let mut s = 0;
                for i in 0..k {
                    let br = b[p[i]] - 1;
                    let from = g.edges[br].from;
                    let to = g.edges[br].to;
                    if dir & (1 << i) == 0 {
                        debug!(s, from, d[s][from]);
                        tt += d[s][from];
                        s = to;
                    }
                    else {
                    ...
                    debug!(g.edges[br].weight);
                    tt += g.edges[br].weight;
                }
                tt += d[s][n - 1];
                debug!(tt);

                tt_min = tt.min(tt_min);
            }
        }
