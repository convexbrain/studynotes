...
fn main() {
    ...
    let p: Vec<u32> = (0..(h * w)).map(|_| {
        token.next().unwrap().parse().unwrap()
    }).collect();

    debug!(p);

    let p2 = Vec2D::from(p, (h, w));

    let mut max_cnt = 0;

    for rbit in 1..=(2_i32.pow(h as u32) - 1) {
        ...
        for c in 0..w {
            ...
            if let Some(tval) = val {
                map.entry(tval).and_modify(|e| *e += 1).or_insert(1);
                ...
                max_cnt = max_cnt.max(map[&tval] * rn);
            }
        }
